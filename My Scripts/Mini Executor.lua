...
-- ─────────────────────────────────────────────────────
-- ROOT GUI — TOP LAYER (Infinite Yield style)
-- ─────────────────────────────────────────────────────

-- 1. Cleanup any old instances hiding in CoreGui or PlayerGui
pcall(function()
    local old = game:GetService("CoreGui"):FindFirstChild("LuaShell4")
    if old then old:Destroy() end
end)
pcall(function()
    local old = PlayerGui:FindFirstChild("LuaShell4")
    if old then old:Destroy() end
end)
...
-- 3. Maximum display order (Infinite Yield uses this exact number)
Root.DisplayOrder   = 2147483647

-- 4. Find the highest possible parent
local function getTopParent()
    -- Priority 1: Exploit's hidden UI (gethui / get_hidden_gui)
    local hiddenUI = (gethui and gethui()) or (get_hidden_gui and get_hidden_gui())
    if hiddenUI then
        return hiddenUI
    end

    -- Priority 2: CoreGui (goes above everything in the game)
    local coreGui = game:GetService("CoreGui")
    if coreGui then
        return coreGui
    end

    -- Fallback: PlayerGui (if everything else fails)
    return PlayerGui
end

-- 5. Parent it to the top layer
Root.Parent = getTopParent()

-- 6. (Optional) Protect the GUI from being deleted/seen by the game
-- This works on Synapse X / some exploits
